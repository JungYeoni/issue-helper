...
- 제목이 이모지로만 구성된 경우 → slug는 기존 로직대로 빈 문자열이 되어 `untitled`로 대체된다 (예: `feat/20260804_#70_untitled`).
...
- 제목이 이모지 제거 후 완전히 빈 문자열이 되는 경우의 커밋 메시지 포맷(`feat:  (#N)`처럼 공백이 이중으로 남는 문제)은 이번 변경 이전부터 존재하던 엣지 케이스이며, 이번 작업 범위에서 다루지 않는다.
