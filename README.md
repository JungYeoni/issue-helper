...
| 브랜치명    | `feat/{날짜:YYYYMMDD}_#{이슈번호}_{slug}` (커밋 타입은 항상 `feat` 고정)                                                               |
| 커밋 메시지 | `feat: {원본 제목} (#{이슈번호})` — 이모지 제거·연속 공백 정리 후 그대로 사용 (slug화하지 않음) · 결과가 빈 문자열이면 `untitled` |
| slug        | 한글 원문 유지(번역/로마자 변환 없음) · 이모지 제거 · 브랜치명에 못 쓰는 문자 제거 · 공백은 `_`로 치환 · 최대 50자 · 결과가 빈 문자열이면 `untitled` |
