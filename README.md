...
이슈가 생성되면 규칙 기반으로 브랜치명·워킹트리명·커밋 메시지를 제안해 이슈 코멘트로 남기는 GitHub Actions 워크플로우.
...
| 워킹트리명  | `{날짜:YYYYMMDD}_#{이슈번호}_{slug}` — 브랜치명과 달리 커밋 타입 접두사가 없다. 폴더명에는 타입 분류가 필요 없고, 브랜치명과 시각적으로 구분되게 하기 위함 (`fix/20260715_..._#123` vs `fix_20260715_..._#123`처럼 슬래시 하나 차이면 헷갈리기 쉬움). `orca worktree create --name <워킹트리명>`처럼 바로 붙여넣을 수 있다 |
