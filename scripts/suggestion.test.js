...
test("워킹트리 이름에는 커밋 타입 접두사가 없다 (브랜치명과 혼동 방지, 폴더명에 분류 불필요)", () => {
  assert.equal(
...
test("워킹트리 이름은 slug의 슬래시 치환 규칙을 그대로 물려받아 슬래시가 남지 않는다", () => {
  const worktree = buildWorktreeName(8, "README/이슈템플릿", "2026-08-12T00:00:00Z");
  assert.ok(!worktree.includes("/"));
  assert.equal(worktree, "20260812_#8_README_이슈템플릿");
});
